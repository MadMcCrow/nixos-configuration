...
let
  ...
  nxc = cts.nextcloud-aio;
in {
  # interface :
  options.nixos.server.containers.nextcloud-aio = {
    enable = lib.mkEnableOption "nextcloud all in one";
  ...
  config = lib.mkIf nxc.enable {
    ...
    virtualisation.oci-containers.containers."nextcloud-aio-mastercontainer" = {
      ...
      # NEXTCLOUD_dataDir = "/mnt/ncdata"; # "${nxc.dataDir}"; # Allows to set the host directory for Nextcloud's dataDir. ⚠️⚠️⚠️ Warning: do not set or adjust this value after the initial Nextcloud installation is done! See https://github.com/nextcloud/all-in-one#how-to-change-the-default-location-of-nextclouds-dataDir
